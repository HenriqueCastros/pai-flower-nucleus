...
class Zoom_Advanced(ttk.Frame):
    ...
    def __init__(
        self, mainframe, img: str | np.ndarray, vbar_col=6, hbar_row=1, colspan=6
    ):
        ...
        vbar.grid(row=0, column=vbar_col, sticky='ns')
        hbar.grid(row=hbar_row, column=0, sticky='we', columnspan=colspan)
        ...
        self.canvas.grid(row=0, column=0, sticky='nswe', columnspan=colspan)
        ...
        if isinstance(img, str):
            ...
            self.image = self.image.resize(
                scale_proportional(self.image.size, (800, 400))
            )
